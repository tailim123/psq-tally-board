(function(){
  ...
  function unresolved(){
    ...
    s.rows.forEach(function(c){
      ...
      var p = s.place.get(c);
      if(p>3 || s.shared.get(c)<2) return;
      var g = s.rows.filter(function(x){ return s.place.get(x)===p; });
      g.forEach(function(x){ seen.add(x); });
      groups.push({place:p, members:g});
    });
  ...
  function renderTop(){
    ...
    var u = unresolved(), flag = $("alertFlag");
    if(u){
      var live = u.some(function(g){ return g.members.some(function(m){ return m.sd.some(function(v){return v;}); }); });
      flag.style.display="";
  ...
  function hasSdIn(g){ return g.members.some(function(c){ return c.sd.some(function(v){return v;}); }); }
  ...
  function renderSd(){
    ...
    var blocks = u.map(function(g){
      ...
      var status;
      if(!hasSdIn(g)) status = '<div class="banner info">Ask the '+state.meta.sdCount+
        ' tie-break questions to these '+g.members.length+' contestants at the same time.</div>';
      else if(alive.length===1) status = '<div class="banner ok"><b>'+esc(alive[0].name)+'</b> takes place '+g.place+'.</div>';
      ...
      else if(done) status = '<div class="banner warn">'+alive.length+' contestants survived all '+
        state.meta.sdCount+' questions. The Board of Judges decides how to proceed.</div>';
      else status = '<div class="banner info">'+alive.length+' still in play.</div>';
  ...
  function sceneHTML(){
    ...
    if(disp.cue==="tiebreak"){
      ...
      var blocks = u.map(function(g){
        ...
        return '<div class="sect">Tie for place '+g.place+'<small>'+grand(g.members[0])+' points each</small></div>'+rows;
      }).join("");
  ...
  function wireScene(root){
    var bs = root.querySelectorAll(".pager .pg");
  ...
  document.addEventListener("input", function(e){
    ...
    if(t.classList && t.classList.contains("rin")){
      ...
      renderTop(); renderTabs(); paintDisplay();
