(function(){
  ...
  /* The introduction runs school by school, alphabetically. A school with one
     coach is one screen; a school with several takes a screen per coach, so the
     pairing the audience sees is the one the operator encoded and never a
     guess. Contestants at such a school with no coach picked come last, on a
     screen of their own — the console has been asking for them all along. */
  ...
  /* The number key is written down rather than taken from the position, so a
     cue can be added without the whole keypad shifting under the operator. The
     champion card is the one cue with no number: R walks the declaration to it,
     and it has its own button beside the reveals. */
  var CUES = [
    ...
    {id:"introduce",   k:"3", t:"Introduce contestants", d:"Portraits, names and schools"},
    {id:"schools",     k:"4", t:"Students & coaches", d:"School by school, A–Z, each with its coach"},
    ...
    {id:"champion",    k:"",  t:"Champion card",     d:"Full-screen winner, on its own"},
  ...
  function renderDesk(){
    ...
    var coachRow = need.length
      ? '<div class="deskrow"><span class="lab">Coaches</span>'+
        ...
        'the students and coaches screen introduces them without one.</span></div>'
      : "";
    ...
    return '<div class="panel"><div class="panel-head"><h3>Display screen</h3>'+
      ...
      '<div class="desk">'+
        ...
        (mech||sch
          ? '<div class="deskrow"><span class="lab">Per page</span><span class="note">'+
            (mech ? 'The mechanics run one screen at a time.'
                  : 'One school at a time, alphabetically. A school that has sent two or more coaches '+
                    'takes a screen per coach, so every contestant is introduced beside the right one.')+
            '</span></div>'
        ...
        '<div class="deskrow"><span class="lab">Page</span>'+
          ...
          '<span class="note">'+(pl
            ? (mech ? (pl.length+" mechanics screens \u00b7 showing screen "+(disp.page+1)+" of "+pages)
              : (sch ? (pl.length+" screen"+(pl.length===1?"":"s")+" \u00b7 school by school \u00b7 showing "+
  ...
  function per(){
    if(disp.cue==="mechanics" || disp.cue==="schools") return 1;
  ...
  function pagedList(){
    ...
    if(disp.cue==="schools") return introGroups();
  ...
  function sceneHTML(){
    ...
    /* Introduction of students and coaches — one school at a time, in
       alphabetical order, each contestant beside the coach who brought them. */
    if(disp.cue==="schools"){
      var gps = introGroups();
      if(!gps.length) return '<div class="dsp">'+dspHead("Students and coaches")+
        '<div class="body"><div class="none">No contestants yet</div></div></div>';
      var pgg = pageOf(gps), g = pgg.slice[0];
      var ns = g.students.length;
      // one row up to four, then two rows — a portrait needs height above all
      var scol = ns>4 ? Math.ceil(ns/2) : ns;
      var coachBar = g.coach
        ? '<div class="coach">'+portrait(g.coach)+
          '<div class="ctxt"><div class="lbl">Coach</div>'+
          '<div class="who">'+esc(g.coach.name)+'</div></div></div>'
        : '';
      return '<div class="dsp">'+dspHead("Students and coaches")+
        '<div class="body" style="justify-content:flex-start;padding-top:calc(var(--u)*2)">'+
        '<div class="sect">'+esc(g.school || "No school given")+
        '<small>'+ns+' contestant'+(ns===1?"":"s")+'</small></div>'+
        '<div class="schl">'+coachBar+
          '<div class="stus" style="grid-template-columns:repeat('+scol+',minmax(0,1fr))">'+
          g.students.map(function(c){
            return '<div class="icard">'+portrait(c)+
              '<div class="itxt"><div class="num">No. '+c.no+'</div>'+
              '<div class="who">'+esc(c.name)+'</div></div></div>';
          }).join("")+'</div>'+
        '</div>'+
        pagerHTML(pgg, 'Screen '+(disp.page+1)+' of '+pgg.pages)+'</div></div>';
    }
